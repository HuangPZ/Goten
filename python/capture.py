...
# def monitor_traffic(interface="eno1", port=None):
#     command = ["sudo", "tcpdump", "-i", interface, "-n", "-l"]
#     if port:
...
#         # Trying to extract packet size
#         # This pattern might need to be adjusted based on your `tcpdump` version output
#         match = re.search(r'length (\d+)', line.decode().strip())
#         if match:
#             print("Packet size: ", match.group(1))

# MasterPort = 29501
# monitor_traffic(port=MasterPort)
...
# Iterate through each interface to get its MAC address
for iface in interfaces:
    try:
        mac_address = get_if_hwaddr(iface)
        all_macs.add(mac_address)
    except Exception as e:
        print(f"Error obtaining MAC for interface {iface}: {e}")
...
def print_pid2traffic():
    ...
    for pid, traffic in pid2traffic.items():
        ...
        if "python" not in name:
            # if the process is not a Python process, then skip it
            continue
        try:
        ...
        with open(log_file_path, "a") as log_file:
            log_file.write(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - PID: {pid}, Name: {name}, Upload: {get_size(traffic[0])}, Download: {get_size(traffic[1])}\n")
